...
async def save_or_update_file(url, page, data, filename="ticket_data.json"):
    try:
        ...
        for ticket in data['ticket_data']:
            raw_section_title, section, row = await process_section_info(ticket['section'])
            row = section if row == "" else row  # Fallback if row is empty
            price = await process_price(ticket['price'])
            
            row_to_add = {"row": row, "price": price, "raw_section_title": raw_section_title, "raw_price": ticket['price'] }
        ...
        for section_name, section_data in existing_data['sections'].items():
            for row in section_data['rows']:
                all_recommendations.append({
                    ...
                    "row": row["row"],
                    "price": row["price"],
                    "raw_section_title": row["raw_section_title"],
        ...
        # Pick the cheapest seat based on recommendations
        for recommendation in sorted_recommendations:
            if 'Sec' in recommendation['raw_section_title'] and 'Row' in recommendation['raw_section_title']:
                existing_data["seat_pick"] = { "raw_section_title": recommendation['raw_section_title'], "raw_price": recommendation['raw_price'] }
                break
